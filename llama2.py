...
def load_model(use_runpod_cache=True, dtype=torch.float32, device="cpu"):
    ...
    hf_model = AutoModelForCausalLM.from_pretrained(weights_source, torch_dtype=dtype, cache_dir=cache_dir)
    ...
    state_dict = loading.get_pretrained_state_dict(tlens_arch, cfg, hf_model=hf_model, dtype=dtype)
